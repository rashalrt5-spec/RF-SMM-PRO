...
const QUICK_PROMPTS = [
  { label: '👑 VIP ও ১০% লাভ', prompt: 'VIP প্যাকেজ ও দৈনিক ১০% অটো লাভ কিভাবে কাজ করে এবং মেয়াদ কতদিন?' },
  { label: '💸 ক্যাশআউট / উইথড্র', prompt: 'বিকাশ বা নগদে ব্যালেন্স উইথড্র / ক্যাশআউট করার নিয়ম ও প্রসেসিং সময় কি?' },
...
export const LiveAISupportModal: React.FC<LiveAISupportModalProps> = ({
  ...
  // Subtle web audio feedback synthesizer
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
      <div className="w-full max-w-lg bg-[#0a1124] border border-amber-500/40 rounded-3xl shadow-[0_10px_50px_rgba(0,0,0,0.8)] flex flex-col h-[90vh] max-h-[700px] overflow-hidden relative text-white">
        ...
        {/* User Context & Shortcuts Bar */}
        <div className="px-3.5 py-2 bg-slate-950/80 border-b border-white/5 space-y-1.5 text-[11px]">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-slate-400">
                অর্ডার: <strong className="text-blue-400 font-mono font-bold">{userTotalOrders}</strong>
              </span>
            </div>
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none py-0.5">
            {onNavigateToVIP && (
              <button
                ...
                className="px-2 py-0.5 rounded-lg bg-amber-500/15 text-amber-300 border border-amber-500/30 hover:bg-amber-500/25 text-[10px] font-bold flex items-center gap-1 shrink-0 active:scale-95 transition cursor-pointer"
              >
                <i className="fas fa-crown text-[9px] text-amber-400"></i>
                <span>VIP ও অটো লাভ</span>
              </button>
            )}
            {onNavigateToWithdraw && (
              <button
                ...
                className="px-2 py-0.5 rounded-lg bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 hover:bg-emerald-500/25 text-[10px] font-bold flex items-center gap-1 shrink-0 active:scale-95 transition cursor-pointer"
              >
                <i className="fas fa-money-bill-wave text-[9px] text-emerald-400"></i>
                <span>ক্যাশআউট</span>
              </button>
            )}
            {onNavigateToDeposit && (
              <button
                ...
                className="px-2 py-0.5 rounded-lg bg-yellow-500/15 text-yellow-300 border border-yellow-500/30 hover:bg-yellow-500/25 text-[10px] font-bold flex items-center gap-1 shrink-0 active:scale-95 transition cursor-pointer"
              >
            ...
            {onNavigateToOrders && (
              <button
                ...
                className="px-2 py-0.5 rounded-lg bg-blue-500/15 text-blue-300 border border-blue-500/30 hover:bg-blue-500/25 text-[10px] font-bold flex items-center gap-1 shrink-0 active:scale-95 transition cursor-pointer"
              >
            ...
            {onNavigateToReferral && (
              <button
                ...
                className="px-2 py-0.5 rounded-lg bg-purple-500/15 text-purple-300 border border-purple-500/30 hover:bg-purple-500/25 text-[10px] font-bold flex items-center gap-1 shrink-0 active:scale-95 transition cursor-pointer"
              >
            ...
            {onNavigateToTasks && (
              <button
                ...
                className="px-2 py-0.5 rounded-lg bg-cyan-500/15 text-cyan-300 border border-cyan-500/30 hover:bg-cyan-500/25 text-[10px] font-bold flex items-center gap-1 shrink-0 active:scale-95 transition cursor-pointer"
              >
                <i className="fas fa-tasks text-[9px]"></i>
                <span>ফ্রি টাস্ক</span>
              </button>
            )}
          </div>
        </div>
                    ...
                    <div className="mt-2.5 pt-2 border-t border-white/10 flex flex-wrap gap-1.5">
                      {(msg.text.includes('VIP') || msg.text.includes('ভিআইপি') || msg.text.includes('অটো লাভ')) && onNavigateToVIP && (
                        <button
                          ...
                          className="px-2.5 py-1 rounded-lg bg-gradient-to-r from-amber-500 to-yellow-400 text-slate-950 font-black text-[10px] flex items-center gap-1.5 shadow-sm hover:brightness-110 active:scale-95 transition cursor-pointer"
                        >
                          <i className="fas fa-crown text-[9px]"></i> VIP প্যাকেজ দেখুন
                        </button>
                      )}
                      {(msg.text.includes('উইথড্র') || msg.text.includes('ক্যাশআউট')) && onNavigateToWithdraw && (
                        <button
                          type="button"
                          onClick={() => {
                            onClose();
                          ...
                          className="px-2.5 py-1 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-white font-black text-[10px] flex items-center gap-1.5 shadow-sm active:scale-95 transition cursor-pointer"
                        >
                          <i className="fas fa-money-bill-wave text-[9px]"></i> ক্যাশআউট করুন
                        </button>
                      )}
                      {(msg.text.includes('ডিপোজিট') || msg.text.includes('Add Funds')) && onNavigateToDeposit && (
                        <button
                          type="button"
                          onClick={() => {
                            onClose();
                          ...
                          className="px-2.5 py-1 rounded-lg bg-yellow-500/20 text-yellow-300 border border-yellow-500/35 hover:bg-yellow-500/30 font-bold text-[10px] flex items-center gap-1.5 active:scale-95 transition cursor-pointer"
                        >
                          <i className="fas fa-wallet text-[9px]"></i> ডিপোজিট করুন
                        </button>
                      )}
                      {(msg.text.includes('অর্ডার') || msg.text.includes('Order')) && onNavigateToOrders && (
                        <button
                          type="button"
                          onClick={() => {
                            onClose();
                          ...
                          className="px-2.5 py-1 rounded-lg bg-blue-500/20 text-blue-300 border border-blue-500/35 hover:bg-blue-500/30 font-bold text-[10px] flex items-center gap-1.5 active:scale-95 transition cursor-pointer"
                        >
                          <i className="fas fa-box text-[9px]"></i> অর্ডার লিস্ট দেখুন
                        </button>
                      )}
                      {(msg.text.includes('WhatsApp') || msg.text.includes('হোয়াটসঅ্যাপ') || msg.text.includes('হোয়াটসএপ')) && (
                        <a
                          href="https://wa.me/8801342163841"
                          target="_blank"
                          rel="noreferrer"
                          className="px-2.5 py-1 rounded-lg bg-emerald-600/30 text-emerald-300 border border-emerald-500/40 font-bold text-[10px] flex items-center gap-1.5 hover:bg-emerald-600/40 active:scale-95 transition"
                        >
                          <i className="fab fa-whatsapp text-[10px] text-emerald-400"></i> WhatsApp এডমিন
                        </a>
                      )}
                      {(msg.text.includes('Telegram') || msg.text.includes('টেলিগ্রাম')) && (
                        <a
                          href="https://t.me/RF2_SMM"
                          target="_blank"
                          rel="noreferrer"
                          className="px-2.5 py-1 rounded-lg bg-sky-600/30 text-sky-300 border border-sky-500/40 font-bold text-[10px] flex items-center gap-1.5 hover:bg-sky-600/40 active:scale-95 transition"
                        >
                          <i className="fab fa-telegram text-[10px] text-sky-400"></i> Telegram সাপোর্ট
                        </a>
                      )}
                    </div>
                  )}
